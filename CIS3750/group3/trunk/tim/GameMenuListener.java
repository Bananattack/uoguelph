...
public class GameMenuListener implements ActionListener, ItemListener
{
	 ...
	 * 
	 * @param c The GameController object
	 * @see GameController
	 */
	public GameMenuListener( GameController c)
	{
		this.controller = c;
	}
	...
	public void actionPerformed( ActionEvent e )
	{
		...
		else if ( selectedMenu.equalsIgnoreCase( "Preferences" ) ) 
		{
			GameSettingsChanger setGame = new GameSettingsChanger( this.controller );
			
			/* in the array air pressure is at position 0 and gravity at 1 */
			int[] temp = setGame.show( controller.getAirPressure(), controller.getGravity()  );
			
			System.out.println("The Air Pressure is now: " + temp[0] + "\nThe Gravity is now: " + temp[1]);
			controller.setAirPressure( temp[0] );
			controller.setGravity( temp[1] );
