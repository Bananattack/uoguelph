...
public class JamieBounce implements Widget
{
	...
	public JamieBounce()
	{
		...
		icon = new ImageIcon("./resources/icons/JamieBounce.gif", "Bounce Ball");
		active = false;
		lock = false;
		bodyArray = new Body[2];
		bodyArray[0] = mainBox;
		bodyArray[1] = mainCircle;
		corner = new Vector2f[4];
		tempX = 0;
		tempY = 0;
		centre = 30;
		boundary = 20;
	...
	public void setPosition(Vector2f f)
	{
		...
		corner[0] = new Vector2f(f.getX(), f.getY());
		corner[1] = new Vector2f(f.getX()+boundary+60+boundary, f.getY());
		corner[2] = new Vector2f(corner[1].getX(), f.getY()+boundary+60+boundary);
		corner[3] = new Vector2f(f.getX(), corner[2].getY());
	}
	...
	{
		return new Vector2f(corner[0].getX(), corner[0].getY());
	}	
