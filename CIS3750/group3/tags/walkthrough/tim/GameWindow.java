...
public class GameWindow extends JFrame implements CollisionListener
{
	...
	/* Phil: works for me */
	private ArrayList<ActiveWidget> levelWidgets = null;
	...
	private GameWindow()
	{
		...
		levelWidgets = new ArrayList<ActiveWidget>();
	...
	void play()
	{
		if(!runningLevel)
		{
			...
			for(ActiveWidget widget : levelWidgets)
			{
	...
	void rotateCC()
	{
		if( activeWidget != null )
			activeWidget.rotateCounterClockwise();
	}
	...
	public void addWidget(ActiveWidget widget)
	{
		...
		//ActiveWidget w = new ActiveWidget();
		//w.addWidget( widget );
	...
	public void updateWidget( )
	{
		...
		if ( this.activeWidget != null && this.mousePosition != null)
		{
			this.activeWidget.setPosition(new Vector2f(  (int) this.mousePosition.getX(), (int) this.mousePosition.getY() ) );
	...
	public void deleteWidget()
	{
		...
		{
			Widget tempDeleteWidget = this.activeWidget;
			float tempX = this.activeWidget.getPositionX();
			float tempY = this.activeWidget.getPositionY();
	...
	public void render(Graphics g)
	{
		if( activeWidget != null && mousePosition != null )
		{
			...
			this.activeWidget.setPosition( new Vector2f( 999999, 999999) );
			/* Phil (2008-10-24): Trying to center widget on mouse */
			this.activeWidget.setPosition(new Vector2f(mousePosition.x - (int) (this.activeWidget.getBoundary()[2].getX() - this.activeWidget.getBoundary()[0].getX()) / 2,
					mousePosition.y - (int) (this.activeWidget.getBoundary()[2].getY() - this.activeWidget.getBoundary()[0].getY()) / 2));

			bounds = this.activeWidget.getBoundary();
			...
			this.activeWidget.draw((Graphics2D) g);
		}
		for(ActiveWidget widget : levelWidgets)
		{
			widget.update();
	...
	public ArrayList<ActiveWidget> getList()
	{
