...
int main(int argc, char** argv)
{
	...
	pthread_barrier_init(&barrier, NULL, W);

	for(size = 0; size < W; size++)
	{
		pthread_create(&tid[size], &attr, ThreadExecute, (void*) size);
	}
	for(i = 0; i < W; i++)
	{
...
void* ThreadExecute(void* arg)
{
	...
	while(1)
	{
        ...
        { 
            ...
            redPercent = redCount * 100 / subregionCells; 
            bluePercent = blueCount * 100 / subregionCells; 
        ...
        {
			lastBlocked = board[y * N] == RED;
			for(i = 0; i < N; i++)
			{
				if(board[y * N + i] == RED && board[y * N + (i + 1) % N] == BLANK)
				{
        ...
        for(x = rcStart; x < rcEnd; x++)
        {
			...
			for(i = 0; i < N; i++)
			{
				if(board[i * N + x] == BLUE && board[((i + 1) % N) * N + x] == BLANK)
				{
					// The last piece needs to check if there was a red on the other side of the board before we started.
					// If there was, don't move it.
					if(i < N - 1 || !lastBlocked)
