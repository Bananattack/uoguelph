...
int PM_hasInitializedAllRequired(ParameterManager* this)
{
	...
	while (iterator != NULL)
	{
		...
		{
			sprintf(PM_error_message, "The required parameter '%s' was not initialized.", current);
			return 0;
		}
...
Boolean __parseListLiteral(char* value, ParameterList** val)
{
	...
	/* Couldn't allocate new list memory */
	if(!l)
	{
		return false;
	}
	...
	for (i = 1, pos = 0; i < strlen(value) - 1; i++)
	{
		...
		if (value[i] == ',')
		{
			...
			listTail(l);
			listAddAfter(l, StringNew(buffer));			
			pos = 0;
		}
...
int PM_parseFrom(ParameterManager* this, FILE* f, char comment_char)
{
	...
	if (!f)
	{
		sprintf(PM_error_message, "Passed a null file handle.");
		return 0;
	}
	...
	while ((c = (char) fgetc(f)) != EOF)
	{
		...
		else if(!in_comment)
		{
			...
			else if (c == '=')
			{
				...
				{
					sprintf(PM_error_message, "Unexpected equals, cannot chain assignment.");
					return 0;
				}
			}
			else if (c == ';')
			{
				if(has_key && i > 0)
				{
					...
					/* 
						Only if the parameter is associated with that name will it be stored.
						Otherwise, ignore it.
					*/
					if(pv)
					{
						switch(pv->type)
						{
							...
							case INT_TYPE:
								...
								{
									sprintf(PM_error_message, "Attempt to assign bad integer value '%s' to parameter '%s'", v, k);
									return 0;
								}
								break;
							case BOOLEAN_TYPE:
								...
								{
									sprintf(PM_error_message, "Attempt to assign bad boolean value '%s' to parameter '%s'", v, k);
									return 0;
								}
								break;
							case REAL_TYPE:
								...
								{
									sprintf(PM_error_message, "Attempt to assign bad real value '%s' to parameter '%s'", v, k);
									return 0;
								}
								break;
							case LIST_TYPE:
								...
								{
									sprintf(PM_error_message, "Attempt to assign bad list value '%s' to parameter '%s'", v, k);
									return 0;
								}
								break;
						}
				...
				{
					sprintf(PM_error_message, "Unexpected semicolon");
					return 0;
				}
			}
			else
			{
				...
				{
					sprintf(PM_error_message, "Delimiter is too long.");
					return 0;
				}
			}
		}
	...
	if(has_key || i > 0)
	{
		sprintf(PM_error_message, "Unexpected end of file.");
		return 0;
	}
