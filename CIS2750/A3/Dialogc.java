...
public class Dialogc extends JFrame implements KeyListener
{
	...
	private ActionListener compileModeAction;
	private Mode compileMode = Mode.IDE;
	...
	public Dialogc()
	{
		...
		fileNewButton.setToolTipText("Create a new configuration file");
		...
		fileOpenButton.setToolTipText("Open a configuration file");
		...
		fileSaveButton.setToolTipText("Save the changes to a configuration file");
		...
		compileCompileButton.setToolTipText("Compile the configuration into a dialog");
		...
		compileCompileAndRunButton.setToolTipText("Compile the configuration into a dialog, and then runs it");
		...
		compileModeAction = new ActionListener()
			{
				public void actionPerformed(ActionEvent e)
				{
					if(e.getActionCommand().equals("ide"))
					{
						compileMode = Mode.IDE;
					}
					if(e.getActionCommand().equals("external"))
					{
						compileMode = Mode.EXTERNAL;
					}
				}
			};
		JMenu configMenuCompileModeMenu = new JMenu("Compile Mode");
		ButtonGroup compileModeGroup = new ButtonGroup();
		configMenu.add(configMenuCompileModeMenu);
		JRadioButtonMenuItem configMenuCompileModeMenuIDEItem = new JRadioButtonMenuItem("Use IDE compiler");
		configMenuCompileModeMenuIDEItem.setActionCommand("ide");
		configMenuCompileModeMenuIDEItem.setSelected(true);
		configMenuCompileModeMenuIDEItem.addActionListener(compileModeAction);
		compileModeGroup.add(configMenuCompileModeMenuIDEItem);
		configMenuCompileModeMenu.add(configMenuCompileModeMenuIDEItem);
		JRadioButtonMenuItem configMenuCompileModeMenuLexYaccItem = new JRadioButtonMenuItem("Use external Lex/Yacc compiler");
		configMenuCompileModeMenuLexYaccItem.setActionCommand("external");
		configMenuCompileModeMenuLexYaccItem.addActionListener(compileModeAction);
		compileModeGroup.add(configMenuCompileModeMenuLexYaccItem);
		configMenuCompileModeMenu.add(configMenuCompileModeMenuLexYaccItem);
	...
	public void compileFile(String filename) throws IOException, ConfigParseException
	{
		...
		if(compileMode == Mode.IDE)
		{
			ConfigCompiler.generate(new File(filenameWithoutExtension), workingDirectory);
		}
		else if(compileMode == Mode.EXTERNAL)
		{
			ConfigCompiler.generateExternal(new File(filenameWithoutExtension), workingDirectory);
		}
		else
		{
			errorMessage("Unknown compile mode.");
			return;
		}
		
		ConfigCompiler.compile(compileCommand.getPath(), compilerArguments);
	}
