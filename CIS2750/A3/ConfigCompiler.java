...
{
	// Parses the .config, converts to .java
	public static void generate(File file, File workingDirectory) throws IOException, ConfigParseException
	{
		...
		// Write the IllegalFieldValue exception
		PrintWriter exceptionWriter = new PrintWriter(new BufferedWriter(new FileWriter(workingDirectory.getPath()+ "/" + file.getName() + "/IllegalFieldValue.java")));
		exceptionWriter.println("public class IllegalFieldValue extends Exception");
		exceptionWriter.println("{");
		exceptionWriter.println("\tpublic IllegalFieldValue() { super(); }");
		exceptionWriter.println("\tpublic IllegalFieldValue(String message) { super(message); }");
		exceptionWriter.println("}");
		exceptionWriter.close();

		// Write the field edit interface!
		PrintWriter interfaceWriter = new PrintWriter(new BufferedWriter(new FileWriter(workingDirectory.getPath()+ "/" + file.getName() + "/" + file.getName() + "FieldEdit.java")));
		interfaceWriter.println("public interface " + file.getName() + "FieldEdit");
		interfaceWriter.println("{");
		// Accessor/mutator methods defined
		for(String field : fields)
		{
			interfaceWriter.println("\tpublic String getDC" + field + "() throws IllegalFieldValue;");
			interfaceWriter.println("\tpublic void setDC" + field + "(String " + field + ");");
			interfaceWriter.println("\t");
		}
		interfaceWriter.println("\tpublic void appendToStatusArea(String message);");
		interfaceWriter.println("}");
		...
		writer.println();
		// Class definition
		writer.println("public class " + file.getName() + " extends JFrame  implements " + file.getName() + "FieldEdit");
		writer.println("{");
		writer.println("\t");
		// Instance variables
		...
		for(String button : buttons)
		{
			...
			writer.println("\t\t" + button + "_button.addActionListener(new " + cm.getStringValue(button) + "(this));");
		...
		for(String field : fields)
		{
			writer.println("\tpublic String getDC" + field + "() throws IllegalFieldValue");
			writer.println("\t{");
			if(cm.getStringValue(field).equals("integer"))
			{
				writer.println("\ttry");
				writer.println("\t{");
				...
				writer.println("\t{");
				writer.println("\t\tthrow new IllegalFieldValue(" + field + "_field.getText());");
				writer.println("\t}");
			}
			else if(cm.getStringValue(field).equals("float"))
			{
				writer.println("\ttry");
				writer.println("\t{");
	...
	// Parses the .config, converts to .java using an external Lex/yacc parser
	public static void generateExternal(File file, File workingDirectory) throws IOException, ConfigParseException
	{
		...
		try
		{
			cmd = "./yadc " + file.getPath() + ".config " + file.getName() + " " + workingDirectory.getName() + "/" + file.getName();
		...
		if (exitCode != 0)
		{
			throw new ConfigParseException("Error during parse:\n" + errorMessage + " " + cmd);
		}
	}

	// Compiles the .java into a .class.
	public static void compile(String compileCommand, String compileArguments) throws IOException, ConfigParseException
	{
		// Now for the fun part -- compile the code using the command they give you.
		int exitCode = 0;
		String errorMessage = "";
